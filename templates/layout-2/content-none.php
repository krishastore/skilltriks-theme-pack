			...
			<div class="stlms-text-xl stlms-p-16 stlms-bg-gray stlms-text-center stlms-text-primary-dark">
				...
				if ( current_user_can( 'edit_post', get_the_ID() ) ) {
					printf(
						...
						esc_html__( 'Ready to publish your first curriculum? %s.', 'skilltriks-theme-pack' ),
						sprintf(
							/* translators: %1$s is Link to new post, %2$s is Get started here */
							'<a href="%1$s" target="_blank">%2$s</a>',
							esc_url( get_edit_post_link( get_the_ID() ) ),
							esc_html__( 'Get started here', 'skilltriks-theme-pack' )
						)
					);
				} else {
					esc_html_e( 'No attached curriculum was found in this course.', 'skilltriks-theme-pack' );
				}
				?>
			</div>
		</div>
